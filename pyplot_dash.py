...
def policy_issued_by_gender_age_product(df):
    try:
        ...
        age_gender_product_policy_count = df.groupby(['age_group', 'insured_gender', 'product']).size().unstack(
            fill_value=0)
...
def total_premium_paid(df):
    try:
        ...
        # find the months when the total premium received is unusually high or low compared to the average.
...
def loss_ratio():
    try:
        ...
        # loss ratio calculation logic , computed as a whole , per year , per product
